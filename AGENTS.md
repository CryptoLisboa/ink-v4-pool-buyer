...
1. **Never broadcast a trade.** Do not run any `SEND=1` command (a live buy/sell) on the
   user's behalf. Executing a swap is the user's action — give them the `SEND=1` command to
   run themselves. Read-only `--dry` quotes are fine for you to run.
2. **Never print, echo, log, or paste the private key.** Not into chat, a file you show, or a
...
3. **Never commit secrets.** `privatekey.txt`, `.env`, `*.key`, `*.seed` are gitignored —
   keep it that way. Before any commit, confirm `privatekey.txt` is not staged and the key
   value appears in no tracked file.
4. **Burner wallet only.** Tell the user to fund this wallet with only what they will trade.
5. **Unverified contracts.** The router and its fee hook are unverified on the explorer — a
...
SEND=1 node buy.mjs amount 0.01 # live (the USER runs this, not the agent)
```
